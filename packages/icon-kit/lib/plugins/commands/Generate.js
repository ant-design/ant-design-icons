        ...
        next: function () {
          var _next2 = _asyncToGenerator(function* (asset) {
            ...
            if (!asset.to.absolute.endsWith('.svg')) {
              api.logger.scope(api.config.name).complete(`Extra file ${_chalk.default.underline.cyan((0, _path.relative)(api.config.context, asset.to.absolute))} is generated.`);
            }
          });
      ...
      var _ref = _asyncToGenerator(function* (args) {
        if (api.assets$ && api.extraAssets$) {
          ...
          if (api.config.destination) {
            yield (0, _fsExtra.emptyDir)(api.config.destination);
          }
          ...
          return new Promise(resolve => {
            ...
            const assets$Subscription = api.assets$.subscribe({
              ...
              complete: () => {
                ...
                if (api.config.destination) {
                  api.logger.scope(api.config.name).complete(`Done. ${count} asset(s) are generated. The sources: ${_chalk.default.underline.cyan('[ ' + api.config.sources + ' ]')}.`);
                } else {
                  api.logger.scope(api.config.name).complete(`Done. There is no file emitted. The sources: ${_chalk.default.underline.cyan('[ ' + api.config.sources + ' ]')}.`);
                }
...
exports.default = GenerateCommandPlugin;