...
export async function build(env: Environment) {
  ...
  // SVG Meta Data Flow
  const svgMetaDataWithTheme$ = from<ThemeType>([ 'fill', 'outline', 'twotone' ]).pipe(
      ...
      ({ identifier, icon }) => {
        ...
        if (typeof icon.icon !== 'function') {
          if (!oldIcons.includes(icon.name)) {
            icon.icon = icon.icon.replace('viewBox="0 0 1024 1024"', 'viewBox="64 64 896 896"');
          }
  ...
  const iconFiles$ = BuildTimeIconMetaData$.pipe(
    map<BuildTimeIconMetaData,
      { identifier: string; content: string; theme: ThemeType }>(({ identifier, icon }) => {
      return {
        identifier,
        theme  : icon.theme,
    ...
    map<{ identifier: string; content: string; theme: ThemeType },
      WriteFileMetaData>(({ identifier, content, theme }) => ({
      path: path.resolve(
  ...
  const indexFile$ = svgMetaDataWithTheme$.pipe(
    ...
    reduce<BuildTimeIconMetaData, string>(
      (acc, { identifier, icon }) => acc + `export { ${identifier} } from './${icon.theme}/${identifier}';\n`,
      ''
    ),
